...
exports.addInstallmentItem = async ({ userId, guestId, productId, quantity }) => {
    if (!mongoose.Types.ObjectId.isValid(productId)) {
        throw new Error("شناسه محصول معتبر نیست");
    }
    ...
    // بررسی اینکه محصول از قبل به صورت اقساطی تو سبد هست یا نه
    let existingItem = cart.items.find(
        (item) => item.product.toString() === productId.toString() && item.type === "installment"
    );
